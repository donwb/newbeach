...
func TestCityVerdictAllOpenQuiet(t *testing.T) {
	...
	out := BuildOutlook(now, ramps, Params{Default: DefaultParams}, preds, nil, nil, nil)
...
func TestCityVerdictNoneOpenHeadlines(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			out := BuildOutlook(now, tc.ramps, Params{Default: DefaultParams}, preds, nil, nil, nil)
...
func TestCityVerdictGolden(t *testing.T) {
	...
	out = BuildOutlook(et(1, 18, 5), ramps, Params{Default: DefaultParams}, preds, nil, nil, nil)
	cv = verdictFor(t, out, "NEW SMYRNA BEACH")
...
func TestCityVerdictOvernight(t *testing.T) {
	...
	out := BuildOutlook(et(1, 21, 0), ramps, Params{Default: DefaultParams}, preds, nil, nil, nil)
	cv := verdictFor(t, out, "NEW SMYRNA BEACH")
	...
	out = BuildOutlook(et(1, 5, 0), ramps, Params{Default: DefaultParams}, preds, nil, nil, nil)
	cv = verdictFor(t, out, "NEW SMYRNA BEACH")
	assert.Equal(t, CityStateOvernight, cv.State)
